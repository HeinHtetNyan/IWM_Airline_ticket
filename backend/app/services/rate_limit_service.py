...
def check_rate_limit(
    key: str, limit: int, window: int, *, fail_open: bool = True
) -> None:
    try:
        ...
        if fail_open:
            logger.exception(
                "Rate limiter unavailable for key %s. Allowing request.", key
            )
            return
...
def enforce_email_rate_limit(
    ...
    *,
    ...
    check_rate_limit(
        build_rate_limit_key(action, email), limit, window, fail_open=fail_open
    )
