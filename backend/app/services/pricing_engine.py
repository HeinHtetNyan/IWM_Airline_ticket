...
def apply_pricing_logic(
    ...
    for flight in api_flights:
        result_flight = dict(flight)
        try:
            base_price_usd = Decimal(str(result_flight["base_price_usd"]))
        except (KeyError, ValueError, TypeError):
            continue

        override = _find_override(
            db,
            result_flight.get("airline_code"),
            result_flight.get("flight_number"),
            result_flight.get("departure_time"),
        )
        ...
        result_flight["base_price_usd"] = _money(base_price_usd)
        result_flight["adults"] = adults
        result_flight["final_price_usd"] = total_price_usd
        result_flight["final_price_mmk"] = total_price_mmk
        result_flight["price_estimate_min_usd"] = price_estimate_min_usd
        result_flight["price_estimate_max_usd"] = price_estimate_max_usd
        result_flight["price_estimate_min_mmk"] = price_estimate_min_mmk
        result_flight["price_estimate_max_mmk"] = price_estimate_max_mmk
        result_flight["requires_admin_confirmation"] = True

        final_flights.append(result_flight)
